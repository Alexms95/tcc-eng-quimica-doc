...
\begin{titlepage}
    ...
    \noindent Orientadores: Heloísa Lajas Sanches Fernandes\\
...
\newpage
\pagenumbering{roman}
\includepdf[pages=-,fitpaper=true]{ficha_catalografica.pdf}
\newpage
...
Aplicando a condição de \emph{estado quasi-estacionário} proposta por Briggs e Haldane (1925), admite-se que a concentração de $ES$ permanece aproximadamente constante após um breve período inicial (estado pré-estacionário). A resolução das equações diferenciais resulta na equação de velocidade
...
No contexto de reações catalisadas por lipases em emulsões óleo-água, o mecanismo ping-pong-bi-bi apresenta-se como modelo cinético fundamental. A lipase sofre uma modificação interfacial ao hidrolisar o primeiro éster graxo, formando o intermediário acil-enzima ($E'$) e liberando um ácido graxo ($P$). Em seguida, $E'$ reage com a porção restante do substrato ($B$), regenerando a forma livre da enzima ($E$) e liberando o segundo produto ($Q$) \cite{nelson2018}.
...
\begin{figure}[H]
    \centering
    \subfloat[Vizinhança de von Neumann de um componente A, definida pelas 4 células azuis.]{%
        \includegraphics[width=0.4\textwidth]{img/vizinhanca_von_neumann.png}
    }\hspace{0.05\textwidth}
    \subfloat[Vizinhança de von Neumann estendida de um componente A, definida pelas células azuis e verdes.]{%
        \includegraphics[width=0.4\textwidth]{img/vizinhanca_von_neumann_estendida.png}
    }\hspace{0.05\textwidth}
    \subfloat[Vizinhança de Moore de um componente A, definida pelas 8 células azuis.]{%
        \includegraphics[width=0.4\textwidth]{img/vizinhanca_moore.png}
    }
...
A Tabela \ref{tab:parametros_fundamentais} resume os parâmetros fundamentais do modelo de autômato celular, incluindo seus símbolos e intervalos de valores possíveis.
...
\begin{table}[H]
    ...
    \label{tab:parametros_fundamentais}
\end{table}
...
Um exemplo clássico da aplicação de autômatos celulares em reações químicas é a simulação de reações de Belousov-Zhabotinsky, que exibem padrões oscilatórios e ondas de propagação. Nesses modelos, as regras de transição são formuladas para replicar as interações químicas que levam a esses comportamentos complexos, e os ACs conseguem reproduzir fielmente os padrões observados experimentalmente. Outras aplicações incluem a simulação de reações em catálise heterogênea, onde a superfície do catalisador pode ser representada por uma grade de células, e as moléculas reagentes se adsorvem, reagem e dessorvem de acordo com as regras do autômato. A flexibilidade dos autômatos celulares permite a incorporação de diversos fatores, como a temperatura e a presença de inibidores ou ativadores, tornando-os uma ferramenta versátil para a exploração de fenômenos cinéticos complexos \cite{kier2005}.

\subsection{Simulação de Reações Enzimáticas com Autômatos Celulares}

Nas últimas décadas a aplicação de autômatos celulares (ACs) a sistemas
enzimáticos evoluiu de provas de conceito, focadas em cinéticas muito simples, para
modelos capazes de reproduzir cenários industriais mais realistas. A seguir, apresenta-se
uma análise dessa trajetória, destacando lacunas que motivam o presente
trabalho.

\subsubsection{Trabalho de Seybold, Kier e Cheng \citeyear{seybold1997simulation}: Prova de Conceito para Cinética de Primeira Ordem}

Um dos trabalhos pioneiros na aplicação de autômatos celulares (ACs) para a cinética química foi o de \textcite{seybold1997simulation}. A principal contribuição dos autores foi demonstrar que um modelo computacional discreto, baseado em regras locais e probabilísticas, poderia reproduzir com fidelidade o comportamento macroscópico de uma reação de primeira ordem ($A \rightarrow B$), que classicamente é descrita por uma equação diferencial ordinária. Em seu modelo, cada célula de uma grade bidimensional poderia assumir o estado de reagente ou produto, e a transição entre esses estados era governada por uma probabilidade derivada diretamente da constante de velocidade da reação.
...
O mérito do trabalho reside na sua capacidade de validar a abordagem de AC como uma ferramenta viável para a simulação de sistemas reacionais. Os resultados mostraram que, para sistemas com um grande número de células, a curva de decaimento da concentração do reagente obtida pelo AC convergia para a solução analítica da equação de primeira ordem. Mais importante, o modelo também capturava as flutuações estocásticas inerentes a sistemas com poucas partículas, um fenômeno que modelos determinísticos contínuos não conseguem representar. Essa capacidade de transitar entre comportamentos determinísticos e estocásticos ressalta a versatilidade do método e sua adequação e evolução para o estudo de processos que apresentam dinâmicas complexas, como as reações enzimáticas.
...
No entanto, para o escopo de um estudo sobre reações enzimáticas, as limitações do modelo de \textcite{seybold1997simulation} são evidentes e cruciais. Primeiramente, o modelo foi desenhado exclusivamente para cinéticas de primeira ordem, não contemplando a complexidade de mecanismos como o de Michaelis-Menten, que envolve múltiplas espécies (enzima, substrato, complexo enzima-substrato) e etapas reacionais com diferentes ordens. Em segundo lugar, o modelo não incluía a dinâmica de movimentação e difusão das partículas no espaço; as células apenas mudavam de estado, mas não representavam moléculas que se moviam e colidiam. Essa ausência de mobilidade impede a simulação de etapas fundamentais do processo enzimático, como o encontro difusional entre a enzima e o substrato para a formação do complexo $ES$.

Portanto, embora tenha estabelecido a base conceitual de que ACs podem simular reações químicas, o trabalho de \textcite{seybold1997simulation} funciona como um ponto de partida. Ele evidencia a necessidade de modelos mais sofisticados que incorporem a movimentação de múltiplas espécies e regras de interação mais complexas para descrever adequadamente a dinâmica espacial e temporal da catálise enzimática, justificando assim a evolução para as abordagens que se seguiram e a proposta do presente trabalho.

\subsubsection{Trabalho de Weimar \citeyear{weimar2002cellular}: A Transição para a Cinética Enzimática}

Dando um passo fundamental além das provas de conceito com reações de primeira ordem, o trabalho de \textcite{weimar2002cellular} demonstrou a capacidade dos autômatos celulares para simular redes de reações enzimáticas, incluindo o mecanismo de Michaelis--Menten. A principal inovação de Weimar foi a introdução explícita da movimentação das espécies químicas (enzimas e substratos) na grade. Essa adição superou uma das limitações centrais dos modelos anteriores, como o de \textcite{seybold1997simulation}, permitindo que a simulação capturasse o processo de encontro difusional entre as moléculas, que é um pré-requisito para a reação enzimática.

Ao modelar a difusão e a reação como eventos locais e probabilísticos, a abordagem de Weimar conseguiu reproduzir o comportamento cinético global esperado para o modelo de Michaelis-Menten. Mais do que isso, o modelo foi capaz de revelar fenômenos espaciais que são inacessíveis a modelos baseados em equações diferenciais ordinárias, como a formação de gradientes de concentração locais e a emergência de padrões decorrentes da interação e do consumo de substrato em diferentes regiões da grade. O trabalho, portanto, validou os ACs não apenas como uma curiosidade matemática, mas como uma ferramenta com potencial para investigar a dinâmica espacial em sistemas bioquímicos.

Contudo, apesar do avanço significativo, a abordagem de Weimar ainda apresentava limitações importantes para a modelagem de sistemas enzimáticos realistas. O modelo, embora incluísse a difusão, não se baseava em um arcabouço físico-químico detalhado para as interações intermoleculares. Parâmetros que governam a afinidade, a atração ou a repulsão entre as diferentes espécies (como as interações hidrofóbicas/hidrofílicas ou a coesão do solvente) não eram explicitamente modelados. A dinâmica era governada primariamente pelas probabilidades de reação no encontro, sem uma representação mais refinada das forças que modulam a mobilidade e a formação de complexos.

Adicionalmente, o modelo de Weimar operava em um ambiente monofásico e homogêneo. Ele não foi projetado para lidar com a complexidade de sistemas multifásicos, como emulsões óleo-água, nem para incorporar o efeito de moléculas anfifílicas como os surfactantes, que alteram drasticamente a dinâmica interfacial e a disponibilidade do substrato.

A contribuição de \textcite{weimar2002cellular} foi demarcar a transição dos ACs de modelos teóricos para ferramentas aplicáveis à cinética enzimática. Ao validar a simulação do mecanismo de Michaelis--Menten em um contexto espacial, o trabalho estabeleceu um novo patamar de complexidade. Contudo, o modelo proposto, ao operar em um ambiente idealizado e monofásico, simultaneamente expôs a lacuna existente entre a simulação de sistemas bioquímicos simplificados e a representação de processos industriais heterogêneos. A ausência de um tratamento para interações intermoleculares mais refinadas e, principalmente, para a dinâmica em interfaces multifásicas, definiu a próxima fronteira de pesquisa na área.

\subsubsection{Trabalho de Dutta \citeyear{dutta2015generalized}: Um Modelo Generalizado para Cinética Enzimática}

Dutta et al. \cite{dutta2015generalized} propõem um modelo de autômatos celulares probabilístico para simular cinéticas enzimáticas de primeira ordem, incorporando inibição e interações estocásticas em uma grade bidimensional com vizinhança estendida de von Neumann e condições de contorno periódicas. O modelo representa agentes como substrato (S), produto (P), enzima (E), inibidor (I) e água (W), com regras probabilísticas para junção (joining), quebra (breaking), transição (transition) e movimento livre (free movement), análogas às constantes cinéticas da equação de Michaelis--Menten. Simulações são realizadas variando parâmetros para analisar sensibilidade, revelando que probabilidades de transição (e.g., de ES para EP) são as mais influentes na concentração final de produto.

Este trabalho avança em relação a estudos anteriores, como os de Seybold et al. \cite{seybold1997simulation}, que focavam em simulações determinísticas ou simplificadas de cinéticas enzimáticas via AC, ao introduzir uma base estocástica mais geral e randômica. A motivação principal parece ser superar limitações de modelos não probabilísticos, permitindo uma representação mais realista de flutuações moleculares e inibições, com análises de sensibilidade que correlacionam variações paramétricas a propriedades cinéticas macroscópicas.

As limitações do modelo podem ser elencadas da seguinte forma:
\begin{enumerate}[label=\alph*)]
    \item \textbf{Formulação simplificada}: O modelo não representa reações enzimáticas específicas, exigindo calibração futura para validação experimental, o que enfraquece sua aplicabilidade imediata \cite{dutta2015generalized}.
    \item \textbf{Alto custo computacional}: Simulações completas demandam até 22 dias de processamento, além da dependência de sementes randômicas fixas, o que reduz a robustez estocástica.
    \item \textbf{Ausência de heterogeneidade espacial}: O modelo assume um meio homogêneo, sem variações espaciais nas propriedades dos agentes ou do meio, ignorando gradientes ou irregularidades comuns em processos enzimáticos reais.
    \item \textbf{Limitação a meios monofásicos}: A abordagem é restrita a uma única fase aquosa implícita (com água como agente), sem representação de interfaces entre fases (emulsões, por exemplo), o que limita sua aplicação a sistemas heterogêneos.
    \item \textbf{Omissão de surfactantes}: Não há inclusão de surfactantes, que poderiam modular interações interfaciais e estabilidade de complexos enzimáticos em ambientes multifásicos, representando uma limitação relevante para simulações mais realistas em engenharia química e de bioprocessos.
\end{enumerate}

Em relação ao presente trabalho, o modelo de Dutta et al. \cite{dutta2015generalized} serve como base inspiradora para o desenvolvimento de software de simulação de processos enzimáticos, particularmente na implementação de ACs estocásticos em ambientes computacionais acessíveis, incorporando certo grau de generalização e, ao mesmo tempo, introduzindo efeitos espaciais heterogênios e divisão de fases, com o estudo do surfactante nesses sistemas.

\subsubsection{Trabalho de Vasconcelos \citeyear{vasconcelos2019}}

O objetivo central do trabalho de Vasconcelos \cite{vasconcelos2019} foi construir um modelo de autômatos celulares (AC) em \textit{Scilab} capaz de reproduzir a cinética enzimática de Michaelis–-Menten e variações mais complexas sem recorrer a equações diferenciais. Para isso, o autor partiu de uma malha quadrada 2-D de 100 × 100 sítios com contorno periódica (toroidal), no qual células das bordas interagem continuamente com as células opostas, de modo que cada célula pode conter exatamente um componente químico ($E$, $S$, $P$, complexos ou “água”) e interagir apenas com seus quatro vizinhos de von Neumann.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img/fluxograma2.png}
...
A escolha de 15000 iterações foi feita para garantir que, próximo do final da execução da simulação, o sistema já tenha pouquíssima ou nenhuma reação, com concentração de substrato próxima de zero, o que indica que a enzima já não está mais reagindo com o substrato. Esse valor foi adotado após observar o nível de completude da reação em diferentes números de iterações, com o objetivo de encontrar um valor que promovesse a estabilização do sistema e, ao mesmo tempo, que não tornasse a execução excessivamente longa.

Uma malha de $100 \times 100$ células (10000 células no total) é suficiente para observar graficamente o comportamento do sistema e ao mesmo tempo simular a presença de uma quantidade considerável de espécies químicas sem comprometer muito o tempo de execução total da simulação. As frações relativas foram escolhidas para simular um experimento real, onde a enzima está presente em quantidade bem abaixo do substrato em um meio aquoso.

Em relação aos parâmetros de reação, definiu-se que sempre que substrato e enzima se encontrarem, eles irão formar o complexo enzima-substrato, entrando com o $P_R$ dessa etapa igual a 1. Esse valor traduz uma reação onde se parte do princípio que a formação do complexo é uma reação muito mais rápida que a sua decomposição (reação inversa) e que a formação de produto, além da própria difusão dos componentes no meio. A reação inversa, onde o complexo se decompõe em enzima e substrato, ocorre com probabilidade $P_R = 0{,}0001$, o que simula uma reação reversível onde a formação do complexo é favorecida. A conversão do complexo em produto ocorre com probabilidade $P_R = 0{,}001$, o que simula a etapa lenta da reação geral. O mecanismo clássico de Michaelis--Menten propõe que a reação entre enzima e produto para formação do complexo não ocorre, portanto, o $P_R$ dessa etapa é 0.
...
Nota-se, pelos mesmos motivos, a boa interação entre enzima e água, além da diferença entre a ligação da enzima com o substrato e dela com o produto. Enzima-substrato foi pensada para ser uma interação maior no sentido de favorecer a formação do complexo, em detrimento da interação com o produto que, segundo a cinética em estudo, não gera reação. O valor de $J (E|P)$ ser um pouco mais baixo também vem da ideia que, após uma reação completa, o produto se separa do sítio ativo da enzima, sugerindo uma interação mais fraca entre eles (com $P_B (E,P)$ um pouco maior, dada a relação inversa entre $J$ e $P_B$).

Os parâmetros $J (S|S)$, $J (P|P)$ e $J (E|E)$ foram definidos como 0, o que significa que não há interação entre células do mesmo tipo, ou seja, tanto enzimas quanto substratos e produtos ficam espalhados no meio arquoso. Isso simula um sistema onde as moléculas de cada um desses componentes estão solubilizados em água, sem formação de aglomerados.
...
Outra análise interessante é a geração da curva de Michaelis-Menten, que relaciona a taxa da reação, que pode ser entendida como a taxa de formação de produto, com a concentração do substrato. Para isso, utilizou-se os dados de concentração do produto ao longo das iterações para fazer uma regressão não-linear, na qual a curva de ajuste é uma equação empírica que reflete a concentração de produto pelo tempo, dada por:
...
O ajuste utilizando uma equação empírica é uma prática comum em Engenharia de Processos, especialmente em ocasiões onde não se conhece uma solução analítica para o problema. Aqui, embora se trate de uma simulação clássica de Michaelis--Menten, a curva de concentração de produto foi ajustada por uma equação empírica a fim de propor um método padrão para a análise dos resultados, inclusive em cenários complexos e heterogêneos, os quais são analisados posteriormente neste trabalho.
...
Comparando esses resultados com os da tabela \ref{tab:params_MM} da simulação de cinética clássica, nota-se que $V_{max}$ é menor e $K_M$ é maior no caso do substrato apolar. A diminuição de $V_{max}$ indica que a taxa máxima de formação de produto cai devido à tendência do substrato em se aglomerar, uma vez que ele desta vez não se solubiliza em água. Levando em consideração que a enzima possui boa afinidade com a água, a reação com o substrato ocorre com menos frequência. A redução da frequência com que a reação enzimática ocorreu também é refletida no aumento de $K_M$.

Embora o modelo de Michaelis-Menten não considere a presença de substratos apolares, os resultados obtidos com o modelo de autômato celular implementado mostram que é possível simular o comportamento da cinética enzimática mesmo em condições não ideais. No cenário em estudo, a teoria que a reação enzimática com substrato situado uma fase diferente da enzima (que torna o substrato pouco disponível) é prejudicada em relação à uma com substrato solúvel pode ser confirmada. Isso demonstra a flexibilidade e a utilidade do modelo para estudar diferentes cenários e condições reacionais.
...
Por outro lado, $K_M$ se apresentou significativamente maior. Isso pode ser explicado pelo fato de que o surfactante, apesar de auxiliar na formação do produto, determinou uma inibição. Pelos parâmetros de interação, tanto $J (E|S)$ quanto $J (E|F_1)$ e $J (E|F_2)$ são iguais a $1{,}1$. Esses valores indicam que o surfactante pode, por muitas vezes, ter agido como um componente que ``competiu'' com o substrato pela atração com a enzima, tornando-a menos disponível para reagir com o substrato. Pode-se entender que
o componente anfifílico agiu teoricamente como um inibidor competitivo, dificultando o encontro entre enzima e substrato ao longo do tempo.

Cabe ressaltar, novamente, que o modelo de Michaelis-Menten não considera a presença de espécies como surfactantes e as constantes calculadas aqui são \textbf{aparentes}. Entretanto, o modelo de AC foi capaz de simular o comportamento da cinética enzimática mesmo com a complexidade adicional do surfactante. A heterogeneidade do sistema e a dinâmica de interação entre os componentes foram bem capturadas.
...
\section{Análise da Tendência de Formação de Micelas}

Dada a capacidade do modelo em simular o comportamento de surfactantes em diferentes concentrações, como foi mencionado nas duas últimas simulações, foram executadas algumas simulações com o intuito de observar a tendência de partição do surfactante, que remete à Concentração Micelar Crítica (CMC). A CMC é a concentração mínima de surfactante necessária para formar micelas em solução, e é um parâmetro crucial na caracterização de surfactantes.
...
\begin{table}[H]
    \centering
    \caption{Parâmetros da Simulação para estudo da tendência de formação de micelas com 3\% de Surfactante}
    \vspace{0.2cm}
    \begin{tabularx}{\textwidth}{X m{5cm}}
        \hline
        \textbf{Parâmetro}               & \textbf{Valor}   \\
...
\begin{table}[H]
    \centering
    \caption{Parâmetros de Movimento para Simulação de tendência de formação de micelas com 3\% de Surfactante}
    \vspace{0.2cm}
    \begin{tabularx}{\textwidth}{X m{5cm}}
        \hline
        \textbf{Parâmetro} & \textbf{Valor} \\
...
\begin{figure}[H]
    ...
    \caption{\small Configuração básica e frações relativas de componentes da simulação para estudo da tendência de formação de micelas com 3\% de surfactante.}
...
\begin{figure}[H]
    ...
    \caption{\small Parâmetros de movimentação dos componentes da simulação para estudo da tendência de formação de micelas com 3\% de surfactante.}
...
\begin{figure}[H]
    ...
    \caption{\small Estado da malha na iteração 3000 da simulação de tendência de formação de micelas com 3\% de surfactante.}
    \label{fig:cmc_3perc}
\end{figure}
...
\begin{figure}[H]
    ...
    \hspace{0.05\textwidth}
    \includegraphics[width=0.15\textwidth]{img/legend_cmc.png}
    \caption{\small Estado da malha na iteração 2000 da simulação de tendência de formação de micelas com 10\% de surfactante.}
...
\begin{figure}[H]
    ...
    \hspace{0.05\textwidth}
    \includegraphics[width=0.15\textwidth]{img/legend_cmc.png}
    \caption{\small Estado da malha na iteração 2000 da simulação de tendência de formação de micelas com 15\% de surfactante.}
...
Em síntese, o trabalho alcançou seus objetivos ao fornecer uma solução computacional inovadora para a simulação de processos enzimáticos em ambientes complexos, contribuindo para o avanço do conhecimento na área e abrindo novas possibilidades para a pesquisa e o ensino em Engenharia Química e Biotecnologia. Recomenda-se, para trabalhos futuros, a implementação de técnicas de paralelização para otimizar o desempenho computacional, a ampliação do modelo para simular outros tipos de reações e interações moleculares, a incorporação de relações mais explícitas com parâmetros físicos e/ou químicos, como o coeficiente de difusão, além do estudo de correlações claras das propriedades físicas reais do sistema com os parâmetros fundamentais do autômato celular.
